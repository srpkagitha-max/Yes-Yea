...
const examMinutes = e => { const spq=Number(e?.secondsPerQuestion||0),qc=Number(e?.questionCount||0); if(spq>0&&qc>0)return Math.max(1,Math.ceil((spq*qc)/60)); const a=new Date(e?.startTime),b=new Date(e?.endTime); return !isNaN(a)&&!isNaN(b)?Math.max(1,Math.round((b-a)/60000)):Number(e?.totalMinutes||e?.questionCount||0); };
